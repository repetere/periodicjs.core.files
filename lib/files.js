...
function generateAssetFromFile(options) {
  const newasset = {};
  const { periodic, file, req = { user: {}, }, include_asset_body, } = options;
  ...
  if (include_asset_body) {
    delete req.body.$loki;
    newasset.attributes._body = req.body;
  }
  if (file.attributes) {
...
async function uploadFile({ fieldname = 'upload_file', renameOptions = {}, uploadDirectoryOptions = {}, user = {}, file, filename, encoding = 'binary', mimetype, include_asset_body, fileBody = {}, }) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      file.on('end', async () => {
        ...
        const newassets = files.map(file => generateAssetFromFile({
          include_asset_body,
          req: { body: fileBody },
          periodic,
          file,
...
function completeFormHandler(options) {
  ...
  let newassets = [];
  let newdocs = [];
  const include_asset_body= req.query.include_asset_body || req.body.include_asset_body || req.headers.include_asset_body;
  // this.periodic = true;
  ...
  files.forEach(file => {
    if (this.acceptedMIMETypes && Array.isArray(this.acceptedMIMETypes) && this.acceptedMIMETypes.indexOf(file.mimetype) === -1) {
      ...
      return next(`Invalid file type. Please upload a ${extensions} file.`);
...
function uploadFileHandler(options = {}) {
  return uploadFile.bind(Object.assign({}, uploadMiddlewareHandlerDefaultOptions, options));
}
...
const downloadAssetMiddlewareHandlerDefaultOptions = {
  periodic: {},
  send_response: true,
};

function downloadAssetMiddleware(req, res, next) {
  const periodic = this.periodic;
  const assetDBName = this.asset_core_data || this.periodic.settings.express.config.asset_core_data;
  const assetDB = this.periodic.datas.get(assetDBName);
      ...
      if (asset.locationtype !== 'local') {
        https.get(asset.fileurl, (remotefileReadStream) => {
          if(sendFileToControllerData){
            ...
            remotefileReadStream.on('data', chunk => {
              req.controllerData.decryptedFile+=chunk.toString();
            })
            remotefileReadStream.on('end',()=>{
              next();
            })
          } else{
            remotefileReadStream.pipe(res);
          }
        });
      } else {
...
function downloadAssetMiddlewareHandler(options) {
  return downloadAssetMiddleware.bind(Object.assign({}, downloadAssetMiddlewareHandlerDefaultOptions, options));
}
...
module.exports = {
  ...
  downloadAssetMiddlewareHandlerDefaultOptions,
  downloadAssetMiddleware,
  downloadAssetMiddlewareHandler,
};