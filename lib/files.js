...
function completeFormHandler(options) {
  ...
  files.forEach(file => {
    if (this.acceptedMIMETypes && Array.isArray(this.acceptedMIMETypes) && this.acceptedMIMETypes.indexOf(file.mimetype) === -1) {
      ...
      let size = '';
      if (this.sizeLimit > 1000000000) {
        size = `${(this.sizeLimit/1000000000).toFixed(1)}GB` 
      } else if (this.sizeLimit > 1000000) {
        size = `${(this.sizeLimit/1000000).toFixed(1)}MB` 
      } else if (this.sizeLimit > 1000) {
        size = `${(this.sizeLimit/1000).toFixed(1)}KB` 
      } else {
        size = `${this.sizeLimit}B` 
      }
